...
local Library = {
	...
	Scheme = {
		...
		AccentColor = Color3.fromRGB(218, 98, 255),
		OutlineColor = Color3.fromRGB(50, 50, 50),
		FontColor = Color3.new(238, 238, 238),
...
do
	...
	function Funcs:AddSpotlightCard(Idx, Info)
		...
		local CardFrame = New("Frame", {
			BackgroundColor3 = "MainColor",
			BackgroundTransparency = 0.5,
			Size = UDim2.new(1, 0, 0, Info.Height or 120),
			Parent = Container,
		})
		table.insert(Library.Corners, New("UICorner", {
			CornerRadius = UDim.new(0, Library.CornerRadius),
		...
		Groupbox:Resize()
		table.insert(Groupbox.Elements, CardObj)
		if Idx then Options[Idx] = CardObj end

		return CardObj
	end

	--// COMPONENT: Clean 2-Line Scroll Feed (Persis Chilli Asli: Anti-Bocor & Lega)
	function Funcs:AddScrollFeed(Idx, Info)
		...
		local FeedScroll = New("ScrollingFrame", {
			BackgroundColor3 = "MainColor",
			BackgroundTransparency = 0.7,
			Size = UDim2.new(1, 0, 0, Info.Height or 230),
		...
		function FeedObj:Clear()
			for _, item in ipairs(ItemsPool) do
				if item.Frame then item.Frame:Destroy() end
			end
			table.clear(ItemsPool)
			UpdateCanvas()
		end
		...
		function FeedObj:AddItem(itemData)
			...
			-- Baris 1: Nama Pet [RARITY] (Anti Bocor dengan TextTruncate)
			...
			-- Baris 2: Rate $/s · Scale · Berat Kg · Mutasi (1 Baris Horizontal Rapi)
			...
			-- Tag Kanan (Misal sisa waktu & persen eram)
			...
			table.insert(ItemsPool, { Frame = RowBtn })
			UpdateCanvas()
		end

		Groupbox:Resize()
		table.insert(Groupbox.Elements, FeedObj)
		if Idx then Options[Idx] = FeedObj end
...
function Library:CreateWindow(WindowInfo)
	...
	do
		...
		local ServerBadge =
			CreateBadge("<font color='" .. AccentHex .. "'>Server:</font> V" .. tostring(game.PlaceVersion))
	...
	do
		local ToggleBtnTexture = "rbxassetid://117688744510818"
